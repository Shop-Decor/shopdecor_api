    ...
    public class MappingProfiles : Profile
    {
        public MappingProfiles()
        {
            ...
            CreateMap<SanPham, GetUserProduct>()
                .ForMember(dest => dest.ColorName, opt => opt.MapFrom(src => src.SanPham_ChiTiets.Where(ct => ct.SoLuong > 0).Select(ct => ct.MauSac.TenMauSac).Distinct().ToList()))
                .ForMember(dest => dest.Color, opt => opt.MapFrom(src => src.SanPham_ChiTiets.Where(x => x.SoLuong > 0).OrderBy(x => x.Gia).Select(x => x.MauSac.TenMauSac).FirstOrDefault()))
                .ForMember(dest => dest.Hinh, opt => opt.MapFrom(src => src.Hinhs.FirstOrDefault().Link))
                .ForMember(dest => dest.gia, opt => opt.MapFrom(src => src.SanPham_ChiTiets.Where(s => s.SoLuong > 0).Any() ? src.SanPham_ChiTiets.Where(s => s.SoLuong > 0).Min(s => s.Gia) : 0))
                ...
                .ForMember(dest => dest.Size, opt => opt.MapFrom(src => src.SanPham_ChiTiets.Where(s => s.SoLuong > 0).OrderBy(s => s.Gia).Select(s => s.KichThuoc.TenKichThuoc).FirstOrDefault()))
                .ForMember(dest => dest.TotalCount, opt => opt.MapFrom(src => src.SanPham_ChiTiets.Where(x => x.SoLuong > 0).Sum(x => x.SoLuong)))
